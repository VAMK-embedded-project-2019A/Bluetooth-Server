...
{
	const std::string	init_str{"ESPP"};
	const vector<char>	init_message(init_str.begin(), init_str.end());
	const std::string	ack_str{"OK"};
	const vector<char>	ack_message(ack_str.begin(), ack_str.end());
	const std::string	ready_str{"READY"};
	const vector<char>	ready_message(ready_str.begin(), ready_str.end());
...
int BtClient::write(const std::vector<char> &ptext)
{
	if(_aes)
	{
		vector<char> init_vector = vamk::getRandom(16);
		vector<char> ctext = _aes->encrypt(ptext, init_vector);

		// append cipher text to iv and send
		vector<char> write_data;
		write_data.insert(write_data.end(), init_vector.begin(), init_vector.end());
		write_data.insert(write_data.end(), ctext.begin(), ctext.end());
		return _socket->write(write_data);
	}
	else
		return _socket->write(ptext);
}
